...
async function _updateChallengeResource (message, isDelete) {
  ...
  if (resourceRole.id === config.SUBMITTER_ROLE_ID && !isTask) {
    if (isDelete) {
      logger.debug(`v4 Unregistering Submitter ${config.CHALLENGE_API_V4_URL}/${_.get(v5Challenge, 'legacyId')}/unregister?userId=${userId}&v5ChallengeId=${challengeId} - ${JSON.stringify(body)}`)
      await helper.postRequest(`${config.CHALLENGE_API_V4_URL}/${_.get(v5Challenge, 'legacyId')}/unregister?userId=${userId}&v5ChallengeId=${challengeId}`, {}, m2mToken)
    } else {
